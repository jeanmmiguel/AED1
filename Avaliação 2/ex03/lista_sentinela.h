...
if(a == NULL || b == NULL|| b->qtde == 0) return;
    No* auxA = a->sentinela->prox;
    No* auxB = b->sentinela;
    int qtdLista = a->qtde + b->qtde;
    ...
    while(i < qtdLista ){
        ...
        if(j >= a->qtde ){
            ...
            //auxilar guardando valor do primeiro valor de b,pq se o j for maior que a quantidade de elementos do a
            //então ele tem o mesmo valor do sentinela, chegou no sentinela
            No* extra = auxB->prox;

            desconecta_no(extra); 
            insere_esquerda(auxA,extra);


        }
        else if(auxB->prox->dado < auxA->dado){
            //auxilar guardando valor do primeiro valor de b pq o valor de b é menor entao tem q inserir no a
            No* extra = auxB->prox;
            ...
            // avancar ponteiro a pq o valor do a já é menor
            auxA = auxA->prox;
            j++;
        }
        i++;
    ...
    a->qtde = qtdLista; // quantidade final de elementos na a 
